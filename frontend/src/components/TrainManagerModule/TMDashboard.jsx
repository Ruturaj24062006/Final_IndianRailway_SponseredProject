...
export default function TMDashboard({
  ...
  // Extract PME and REF details from profileData
  const pmeRaw = profileData?.pme_status || profileData?.pmeStatus || "FIT";
  const pmeStatus = pmeRaw.includes(" - ") ? pmeRaw.split(" - ")[0] : pmeRaw;
  const pmeDueDate = profileData?.pme_next_due_date || (pmeRaw.includes("Due: ") ? pmeRaw.split("Due: ")[1] : "2029-08-20");
  const pmeDoneDate = profileData?.pme_date || "2025-08-20";

  const refRaw = profileData?.ref_status || profileData?.refStatus || "COMPLETED";
  const refStatus = refRaw.includes(" - ") ? refRaw.split(" - ")[0] : refRaw;
  const refDueDate = profileData?.ref_next_due_date || (refRaw.includes("Due: ") ? refRaw.split("Due: ")[1] : "2027-05-12");
  ...
  const showDetail = (type) => {
    ...
    if (type === "latest_score") {
      ...
      content = (
        <div>
          ...
          <div style={{ marginTop: "16px", background: "#eff6ff", padding: "12px 16px", borderRadius: "8px", border: "1px solid #bfdbfe", fontSize: "13px", color: "#1e3a8a" }}>
            <strong>Safety Goal:</strong> Continuous counseling and refresher course updates are recommended for any staff scoring below 80% to ensure zero-accident train operations.
          </div>
        </div>
      );
      ...
      content = (
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginTop: "14px" }}>
            {[
              { cat: "A", range: "80% - 100%", desc: "Outstanding competency. Fit for mainline, passenger, and critical train operations.", bg: "#dcfce7", color: "#16a34a" },
              { cat: "B", range: "50% - 79%", desc: "Satisfactory operations. Allowed for secondary yard shunting and block siding duties.", bg: "#dbeafe", color: "#2563eb" },
              { cat: "C", range: "26% - 49%", desc: "Needs Improvement. Restricted to supervised duties; mandatory schedule for counseling.", bg: "#fef3c7", color: "#d97706" },
              { cat: "D", range: "Under 26%", desc: "Unfit. Taken off active safety duties immediately. Retraining required.", bg: "#fee2e2", color: "#dc2626" }
            ].map(item => (
      ...
      content = (
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div>
            ...
            <p>Every safety-category staff member (including Train Manager) must clear the PME to confirm physical and sensory fitness (specifically Category A-2 eye fitness for mainline signaling and train operations).</p>
          ...
          <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: "12px" }}>
            ...
            <p>Refresher training must be completed periodically at the Zonal Training Center (ZRTI) to verify knowledge of new rules, block operations, train management, and emergency operations.</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", background: "#f8fafc", padding: "10px", borderRadius: "6px", border: "1px solid #cbd5e1", marginTop: "8px", fontSize: "12px" }}>
      ...
      content = (
        <div>
          <p style={{ marginBottom: "12px" }}>Your safety scores plotted across the last 6 months shows progression in your rule compliance:</p>
  ...
  return (
    <div className="pm-dashboard-layout">
      ...
      <div className="pm-summary-cards">
        <article className="pm-sum-card" style={{ cursor: "pointer" }} onClick={() => showDetail("latest_score")}>
        ...
        <article className="pm-sum-card" style={{ cursor: "pointer" }} onClick={() => showDetail("average_score")}>
        ...
        <article className="pm-sum-card" style={{ cursor: "pointer" }} onClick={() => showDetail("current_category")}>
        ...
        <article className="pm-sum-card" style={{ cursor: "pointer" }} onClick={() => showDetail("total_attempts")}>
      ...
      {/* PME & REF Card for Train Manager */}
      ...
      <div className="pm-charts-row">
        ...
        <div className="pm-chart-card" style={{ cursor: "pointer" }} onClick={() => showDetail("trend_chart")}>
        ...
        {/* Category Pie Chart */}
        <div className="pm-chart-card" style={{ cursor: "pointer" }} onClick={() => showDetail("grade_chart")}>
          <div className="pm-chart-header">
            <BarChart2 size={16} />
